  ...
  for(game_round in 0 : 30) {
    ...
    if(game_round > 0) {
      ...
      match_expectations <- calculate_match_expectations(football_data[seq(1, nrow(football_data), by = 2), ], goal_expectations, 1.35, competition_parameters$home_advantage) %>%
        ...
        select(HomeTeam, AwayTeam, Wedstrijdnummer, Competitie, Seizoen, HomePoints, AwayPoints)
      
      football_data <- football_data %>%
        full_join(match_expectations, by = c("HomeTeam", "AwayTeam", "Wedstrijdnummer", "Competitie", "Seizoen"))
      football_data$HomePoints[seq(2, nrow(football_data), by = 2)] <- football_data$AwayPoints[seq(1, nrow(football_data), by = 2)]
      football_data$AwayPoints[seq(2, nrow(football_data), by = 2)] <- football_data$HomePoints[seq(1, nrow(football_data), by = 2)]
      ...
      aggregated_football_data_to_come <- use_function_with_caching(aggregated_football_data_cache,
                                                                    ...
                                                                    namen) %>%
        mutate(Punten = Punten + calculate_predicted_outcomes(football_data) - calculate_predicted_outcomes(football_data_to_come))
      ...
      aggregated_football_data_passed <- use_function_with_caching(aggregated_football_data_cache,
                                                                   ...
                                                                   namen) %>%
        mutate(Schedule = calculate_predicted_outcomes(football_data) - calculate_predicted_outcomes(football_data_passed))
